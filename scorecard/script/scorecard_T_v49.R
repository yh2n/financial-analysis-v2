...
if(file.exists(pricefile)) {
  ...
  price_list <- get_prices(tickers=all_tks, start=start_date, types=c("Ad", "Hi"), end=end_date, datasource=datasource, outfiles=c(pricefile, hipricefile))
  prices = price_list["Ad"]
  hiprices = price_list["Hi"]
}
...
# rearrange columns
stats_df <- move_col_after(stats_df, "Avg Annualized Returns", "Ticker")
